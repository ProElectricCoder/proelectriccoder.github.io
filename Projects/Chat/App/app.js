/**
 * app.js — P2P Chat Application v3.0
 * ProElectricCoder · /Projects/Chat/App/app.js
 *
 * Features: multi-chat sessions, IndexedDB persistence, 6 per-chat themes with
 * cross-fade transitions, group chat (relay), voice/video/screen calling with
 * full overlay UI, SVG file-type icons, mobile slide-over sidebar.
 */
...
// SVG file-type icons  { svg, color, bg }
const FICONS = {
...
function fileIconHTML(mime) {
	...
	return `<div class="file-icon-wrap" style="background:${fi.bg}">
		<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
			stroke="${fi.color}">${fi.svg}</svg></div>`;
}
...
const DB = {
	...
	async saveMessage(m) {
		const db = await this._open();
		return this._p(db.transaction('messages','readwrite').objectStore('messages').put(m));
	},
	async getSessions() {
		const db = await this._open();
...
const S = {
	...
	encEnabled:     false,
	cubicGradFn:    null,
	gradActive:     1,          // which grad-bg div is visible (1 or 2)
	callSessId:     null,       // session currently in a call
	callTimer:      null,
	callStarted:    null,
	filterQ:        '',
};
...
function makeSess(opts) {
	return {
		id:          opts.id          || 'sess_'+Date.now()+'_'+Math.random().toString(36).slice(2),
		name:        opts.name        || 'New Chat',
		type:        opts.type        || 'firebase',   // 'direct' | 'firebase'
		...
		engine:      null,
		connected:   false,
		peers:       new Map(),       // peerId → { name, avatar }
		messages:    [],              // { id, type, content, sender, mine, timestamp, enc, file }
		unread:      0,
		inFiles:     new Map(),       // transferId → { meta, chunks }
		call: { mediaPc:null, localStream:null, type:null,
...
async function selectSess(id, closeSidebar = true) {
	...
	const prev = getActiveSess();
	if (prev) { prev.unread = 0; renderChatList(); }
	S.activeId = id;
	sess.unread = 0;
...
function setThemeForSess(sessId, themeId) {
	const sess = S.sessions.get(sessId);
	if (!sess) return;
	sess.theme = themeId;
	DB.saveSession(sess);
	DB.updateSession(sessId, { theme: themeId });
	if (S.activeId === sessId) applyTheme(themeId);
	renderChatList();
}
...
function bindEngine(sess, engine) {
	...
	engine.onPeerConnected(peerId => {
		sess.connected = true;
		sess.peers.set(peerId, { name: 'Peer', avatar: '' });
		...
		// For groups: notify all other peers about the new member
	...
	switch (data.type) {
		...
		case 'group-members':
			for (const m of (data.members||[])) {
				if (!sess.peers.has(m.pid)) sess.peers.set(m.pid, { name:m.name, avatar:'' });
			}
			if (S.activeId === sess.id) renderTopbar(sess);
			break;
		...
		case 'chat': {
			let txt = data.text;
			let enc = !!data.encrypted;
			if (enc) {
				if (!Crypt.key) { addBubble(sess, '[🔒 Encrypted — set same password in Settings]', data.displayName||'Peer', false, false); return; }
				try { txt = await Crypt.decText(txt); }
				catch { addBubble(sess, '[⚠ Decryption failed]', data.displayName||'Peer', false, false); return; }
			}
			addBubble(sess, txt, data.displayName||peerName(sess, peerId), false, enc);
			break;
		}

		case 'file-meta':
			sess.inFiles.set(data.id, { meta:data, chunks:[] });
			addSysMsg(sess, `${data.displayName||peerName(sess,peerId)} sending ${data.name} (${fmtSz(data.size)})`);
			break;

		case 'file-chunk': {
			const f = sess.inFiles.get(data.id);
			if (f) f.chunks.push(data.data);
			break;
		}

		case 'file-done':
			await receiveFile(sess, data.id, peerId);
			break;
		...
		case 'call-offer':
			...
			sess.call.incoming = data;
			sess.call.state    = 'ringing';
			S.callSessId       = sess.id;
			showIncomingDialog(sess, data);
			break;

		case 'call-answer':
			if (sess.call.mediaPc && sess.call.state === 'calling') {
				await sess.call.mediaPc.setRemoteDescription(
		...
		case 'call-end':
			...
			addSysMsg(sess, 'Call ended by peer');
			break;

		case 'call-reject':
			endCallInternal(sess, false);
			addSysMsg(sess, 'Call declined');
			break;

		case 'display-name': {
			const old = sess.peers.get(peerId)?.name || 'Peer';
			if (sess.peers.has(peerId)) sess.peers.get(peerId).name = data.displayName;
			addSysMsg(sess, `${old} → ${data.displayName}`);
			break;
		}
	}
}

function peerName(sess, peerId) {
	return sess.peers.get(peerId)?.name || 'Peer';
}

function safeSend(sess, data) {
...
function renderChatList() {
	...
	container.innerHTML = items.map(s => {
		...
		const unreadBadge = s.unread > 0
			? `<div class="ci-badge">${s.unread > 99 ? '99+' : s.unread}</div>` : '';
...
function renderTopbar(sess) {
	const avEl   = el('topbarAv');
	const nameEl = el('topbarName');
	const dotEl  = el('statusDot');
	const txtEl  = el('statusText');
	if (!avEl) return;
	const th = THEMES[sess.theme] || THEMES.void;
	avEl.textContent    = sess.name.slice(0,2).toUpperCase();
	avEl.style.color    = th.primary;
	nameEl.textContent  = sess.name;
	dotEl.className     = 'status-dot ' + (sess.connected ? 'connected' : '');
	txtEl.textContent   = sess.connected
		? `${sess.peers.size} peer${sess.peers.size!==1?'s':''} connected`
		: 'Disconnected';
...
function enableCallBtns(on) {
	['btnVoice','btnVideo','btnScreen'].forEach(id => {
...
async function renderMessages(sess) {
	...
	if (!sess.messages.length) {
		sess.messages = await DB.getMessages(sess.id);
	}
...
function renderMsgItem(container, m) {
	if (m.type === 'system') {
		...
		d.className = 'sys-msg'; d.textContent = m.content;
		container.appendChild(d); return;
	}
	if (m.type === 'file') {
		renderFileBubbleFromMsg(container, m); return;
	}
...
function addBubble(sess, text, sender, mine, enc = false) {
	const m = {
		...
		sessionId: sess.id, type: 'text', content: text,
		sender, mine, timestamp: Date.now(), enc,
	};
	sess.messages.push(m);
	sess.lastMessage  = text.slice(0, 60);
...
function addSysMsg(sess, text) {
	const m = {
		...
		sessionId:sess.id, type:'system', content:text,
		sender:'', mine:false, timestamp:Date.now(), enc:false,
	};
	sess.messages.push(m);
	DB.saveMessage(m);
	if (S.activeId === sess.id) {
...
async function sendFile(file) {
	...
	safeSend(sess, { type:'file-meta', id, name:file.name, size:file.size,
		mime:file.type||'application/octet-stream', chunks, encrypted:isEnc, displayName:S.displayName });
	for (let i=0; i<chunks; i++) {
		safeSend(sess, { type:'file-chunk', id, index:i, data:b64.slice(i*CHUNK_SIZE,(i+1)*CHUNK_SIZE) });
		if (i%8===0) await sleep(0);
	}
	safeSend(sess, { type:'file-done', id });
	// Show sent bubble
	const origBuf = await file.arrayBuffer();
	addFileBubble(sess, { name:file.name, size:file.size, mime:file.type||'application/octet-stream' },
		URL.createObjectURL(new Blob([origBuf],{type:file.type})), true, isEnc);
}

async function receiveFile(sess, id, peerId) {
	const entry = sess.inFiles.get(id);
	if (!entry) return;
	sess.inFiles.delete(id);
	const { meta, chunks } = entry;
	let buf = b64Buf(chunks.join(''));
	if (meta.encrypted) {
		if (!Crypt.key) { addSysMsg(sess, `⚠ Cannot decrypt ${meta.name} — no key`); return; }
	...
	addFileBubble(sess, meta, URL.createObjectURL(blob), false, meta.encrypted);
}

function addFileBubble(sess, meta, url, mine, enc) {
	const m = {
		id:'file_'+Date.now()+'_'+Math.random().toString(36).slice(2),
		sessionId:sess.id, type:'file', content:meta.name,
		sender: mine ? S.displayName : peerName(sess,'remote'),
		mine, timestamp:Date.now(), enc,
		file:{ name:meta.name, size:meta.size, mime:meta.mime,
			dataUrl: meta.mime.startsWith('image/') ? url : null },
	};
	sess.messages.push(m);
	sess.lastMessage  = `📎 ${meta.name}`;
	sess.lastActivity = m.timestamp;
	DB.saveMessage(m);
	DB.updateSession(sess.id, { lastMessage:sess.lastMessage, lastActivity:sess.lastActivity });
	...
	if (S.activeId === sess.id) {
		const c = el('messages');
		if (c) { renderFileBubbleFromMsg(c, m); c.scrollTop = c.scrollHeight; }
	}
	renderChatList();
}
...
function renderFileBubbleFromMsg(container, m) {
	...
	if (meta?.mime?.startsWith('image/') && url) {
		body = `<img src="${url}" class="preview-img" alt="${escH(meta.name)}"
			onclick="App.openLightbox('${url}')" style="cursor:zoom-in">`;
	} else if (meta?.mime?.startsWith('audio/') && url) {
		body = `<audio controls src="${url}" style="max-width:250px;width:100%;margin:2px 0"></audio>
			<div style="font-size:.7rem;font-family:'JetBrains Mono',monospace;color:var(--faint);margin-top:4px">${escH(meta.name)}</div>`;
	} else if (meta?.mime?.startsWith('video/') && url) {
		body = `<video controls src="${url}" style="max-width:270px;width:100%;border-radius:8px;display:block"></video>`;
	} else {
		body = `<div class="file-card">${fileIconHTML(meta?.mime||'')}
			<div style="flex:1;min-width:0">
				<div class="file-name">${escH(meta?.name||m.content)}</div>
				<div class="file-size">${fmtSz(meta?.size||0)}</div>
			</div>
			${url ? `<a href="${url}" download="${escH(meta.name)}" class="btn btn-s" style="padding:5px 10px;font-size:.68rem;flex-shrink:0">↓</a>` : ''}
		</div>`;
	}
	const d = document.createElement('div');
	d.className = `msg ${side}`;
	d.innerHTML = `<div class="msg-meta">${escH(m.sender)} · ${fmtTime(m.timestamp)}${enc}</div>
		<div class="msg-bubble" style="padding:6px">${body}</div>`;
	container.appendChild(d);
}
...
function buildMediaPC(sess) {
	...
	pc.onicecandidate = evt => {
		if (evt.candidate) safeSend(sess, { type:'call-ice', candidate:evt.candidate.toJSON() });
	};
	pc.ontrack = evt => {
		const rv = el('callRemoteVid');
		if (rv) { rv.srcObject = evt.streams[0]; rv.style.display='block'; }
		const ab = el('callAudioBg');
	...
	pc.onconnectionstatechange = () => {
		...
		if (pc.connectionState === 'failed') {
			endCallInternal(sess, true); toast('Call connection failed');
		}
	};
	return pc;
}
...
async function acceptCall() {
	...
	try {
		...
		const stream = await getStream(answerType);
		sess.call.localStream = stream;
		sess.call.mediaPc     = buildMediaPC(sess);
		stream.getTracks().forEach(t => sess.call.mediaPc.addTrack(t, stream));
...
function endCallInternal(sess, notify = true) {
	if (!sess) { sess = S.sessions.get(S.callSessId); }
	if (!sess) return;
	if (notify && sess.connected && sess.call.state !== 'idle')
		safeSend(sess, { type:'call-end' });
	closeIncomingDialog();
	hideCallOverlay();
	stopCallTimer();
	sess.call.localStream?.getTracks().forEach(t => t.stop());
	try { sess.call.mediaPc?.close(); } catch {}
	sess.call = { mediaPc:null, localStream:null, type:null,
		state:'idle', muted:false, camOff:false, incoming:null };
...
function toggleCallMute() {
	...
	const btn = el('callMuteBtn');
	if (btn) {
		btn.classList.toggle('active', sess.call.muted);
		btn.title = sess.call.muted ? 'Unmute' : 'Mute';
	}
}
function toggleCallCam() {
	...
	const btn = el('callCamBtn');
	if (btn) { btn.classList.toggle('active', sess.call.camOff); btn.title = sess.call.camOff ? 'Cam on':'Cam off'; }
}
function stopScreenShare() { const s=S.sessions.get(S.callSessId); if(s) { endCallInternal(s,true); addSysMsg(s,'Screen share ended'); } }
...
function showCallOverlay(sess, localStream) {
	...
	if (lv) {
		if (localStream && (type==='video'||type==='screen')) {
			lv.srcObject = localStream; lv.classList.add('visible');
	...
	el('callCamBtn')?.classList.toggle('hidden', type !== 'video');
	el('callScreenBtn')?.classList.toggle('hidden', type !== 'screen');
...
function stopCallTimer() {
	...
	S.callStarted=null; const t=el('callTimer'); if(t) t.textContent='00:00';
}

// ═══════════════════════════════════════════════════════════════════════════
// 13. INJECTED PANELS (CSS + HTML)
// ═══════════════════════════════════════════════════════════════════════════
function injectPanels() {
	const css = document.createElement('style');
	css.textContent = `
/* ── Settings + Info panels (slide-in right) ── */
.panel-overlay{position:fixed;inset:0;z-index:400;background:rgba(0,0,0,.5);
	backdrop-filter:blur(4px);opacity:0;pointer-events:none;transition:opacity .25s}
.panel-overlay.open{opacity:1;pointer-events:auto}
.panel-drawer{position:fixed;top:0;right:0;bottom:0;width:320px;max-width:94vw;
	background:rgba(5,5,18,.97);border-left:1px solid rgba(0,255,255,.14);
	display:flex;flex-direction:column;overflow:hidden;
	transform:translateX(30px);transition:transform .25s ease}
.panel-overlay.open .panel-drawer{transform:translateX(0)}
.panel-head{display:flex;align-items:center;justify-content:space-between;
	padding:15px 18px 12px;border-bottom:1px solid rgba(0,255,255,.1)}
.panel-head-title{font-family:'Syne',sans-serif;font-size:1rem;font-weight:800}
.panel-close{width:26px;height:26px;border-radius:6px;border:1px solid rgba(255,255,255,.1);
	background:rgba(255,255,255,.04);color:var(--dim);cursor:pointer;display:flex;
	align-items:center;justify-content:center;font-size:1rem;transition:all .15s}
...
.panel-section-lbl{font-family:'JetBrains Mono',monospace;font-size:.59rem;letter-spacing:.2em;
	text-transform:uppercase;color:rgba(0,255,255,.5);margin-bottom:12px}
...
.modal-box{background:rgba(5,5,18,.98);border:1px solid rgba(0,255,255,.18);border-radius:14px;
	width:100%;max-width:420px;max-height:90vh;display:flex;flex-direction:column;overflow:hidden}
.modal-head{display:flex;align-items:center;justify-content:space-between;
	padding:16px 18px 12px;border-bottom:1px solid rgba(255,255,255,.07)}
...
@media(max-width:700px){
	...
	.modal-box{border-radius:14px 14px 0 0;max-width:100%;max-height:88vh}
}

/* ── Call overlay ── */
.call-overlay{position:fixed;inset:0;z-index:600;background:#000;
	display:none;flex-direction:column}
.call-overlay.active{display:flex}
.call-remote-vid{width:100%;height:100%;object-fit:cover;display:none;position:absolute;inset:0}
.call-audio-bg{flex:1;display:flex;flex-direction:column;align-items:center;
...
.call-ring{position:absolute;inset:0;border-radius:50%;border:2px solid rgba(0,255,255,.3);
	animation:callring 2.4s ease-out infinite}
.call-ring:nth-child(2){animation-delay:.8s}
.call-ring:nth-child(3){animation-delay:1.6s}
@keyframes callring{0%{transform:scale(1);opacity:.7}100%{transform:scale(2.8);opacity:0}}
.call-audio-av{width:80px;height:80px;border-radius:50%;background:rgba(0,255,255,.1);
	border:2px solid rgba(0,255,255,.3);display:flex;align-items:center;justify-content:center;
	font-family:'Syne',sans-serif;font-size:2rem;font-weight:800;color:var(--tp);z-index:1}
.call-audio-peer{font-family:'Syne',sans-serif;font-size:1.1rem;font-weight:800;z-index:1}
.call-audio-status{font-family:'JetBrains Mono',monospace;font-size:.75rem;
	color:rgba(232,237,248,.5);z-index:1}
.call-timer-wrap{font-family:'JetBrains Mono',monospace;font-size:.9rem;
	color:rgba(232,237,248,.7);letter-spacing:.06em;z-index:1}
.call-local-vid{position:absolute;bottom:100px;right:14px;width:130px;height:98px;
	border-radius:10px;overflow:hidden;border:1px solid rgba(255,255,255,.15);
	background:#111;object-fit:cover;display:none;z-index:2}
.call-local-vid.visible{display:block}
.call-badge{position:absolute;top:14px;left:50%;transform:translateX(-50%);
	font-family:'JetBrains Mono',monospace;font-size:.65rem;letter-spacing:.1em;
	background:rgba(0,0,0,.6);border:1px solid rgba(255,255,255,.15);
	border-radius:20px;padding:4px 13px;color:rgba(232,237,248,.7);z-index:3}
.call-controls{position:absolute;bottom:0;left:0;right:0;display:flex;align-items:center;
	justify-content:center;gap:18px;padding:22px 16px;
	background:linear-gradient(to top,rgba(0,0,0,.9),transparent);z-index:4}
.call-ctrl{width:56px;height:56px;border-radius:50%;display:flex;align-items:center;
	justify-content:center;border:none;cursor:pointer;transition:all .18s}
.call-ctrl svg{width:24px;height:24px}
.call-ctrl.mute{background:rgba(255,255,255,.14);color:#e8edf8}
.call-ctrl.cam{background:rgba(255,255,255,.14);color:#e8edf8}
.call-ctrl.end{background:#ff4455;color:#fff}
.call-ctrl.screen{background:rgba(255,255,255,.14);color:#e8edf8}
.call-ctrl.active{background:rgba(255,68,85,.25);color:#ff4455}
.call-ctrl:hover{filter:brightness(1.18);transform:scale(1.06)}

/* ── Incoming call ── */
.incoming-overlay{position:fixed;inset:0;z-index:700;background:rgba(0,0,0,.6);
	backdrop-filter:blur(6px);display:none;align-items:center;justify-content:center}
.incoming-overlay.active{display:flex}
.incoming-box{background:rgba(5,5,18,.98);border:1px solid rgba(0,255,255,.2);border-radius:16px;
	padding:30px 26px;text-align:center;display:flex;flex-direction:column;align-items:center;
	gap:14px;max-width:280px;width:90%;animation:inpulse 1.5s ease-in-out infinite}
@keyframes inpulse{0%,100%{box-shadow:0 0 25px rgba(0,255,255,.08)}
	50%{box-shadow:0 0 55px rgba(0,255,255,.2)}}
.incoming-big-icon{font-size:3rem}
.incoming-caller{font-family:'Syne',sans-serif;font-size:1.1rem;font-weight:800}
.incoming-type{font-size:.78rem;color:var(--dim)}

/* ── Lightbox ── */
#lightbox{position:fixed;inset:0;z-index:800;background:rgba(0,0,0,.92);
	display:none;align-items:center;justify-content:center;cursor:zoom-out}
#lightbox.open{display:flex}
#lightbox img{max-width:95vw;max-height:95vh;border-radius:8px;object-fit:contain}

/* ── Theme picker popover ── */
#themePicker{position:fixed;z-index:450;background:rgba(5,5,18,.97);
	border:1px solid rgba(0,255,255,.2);border-radius:11px;padding:14px 16px;
	display:none;flex-direction:column;gap:10px;box-shadow:0 8px 32px rgba(0,0,0,.5)}
#themePicker.open{display:flex}
.tp-title{font-family:'JetBrains Mono',monospace;font-size:.6rem;letter-spacing:.16em;
	text-transform:uppercase;color:var(--faint)}

/* ── Slider ── */
input[type=range].sp-range{flex:1;-webkit-appearance:none;height:3px;
	background:rgba(255,255,255,.08);border-radius:3px;outline:none;cursor:pointer}
input[type=range].sp-range::-webkit-slider-thumb{-webkit-appearance:none;width:13px;height:13px;
	border-radius:50%;background:var(--tp);cursor:grab}
.sp-val{font-family:'JetBrains Mono',monospace;font-size:.7rem;color:var(--tp);
	background:rgba(0,255,255,.08);padding:2px 7px;border-radius:4px;min-width:30px;text-align:center}
`;
	document.head.appendChild(css);

	// ── SETTINGS PANEL ──
	const sp = document.createElement('div');
	sp.id='settingsOverlay'; sp.className='panel-overlay';
	...
	<div class="panel-body">
		<div class="panel-section">
			...
			<div style="display:flex;align-items:center;gap:12px;margin-bottom:12px">
				...
				<div style="flex:1;min-width:0">
					...
					<input class="f-in" id="spName" placeholder="Your name…" maxlength="32">
				</div>
			</div>
		...
		<div class="panel-section">
			...
			<div class="sp-row" style="margin-bottom:12px">
				<span class="sp-label">Enable encryption</span>
				<label class="toggle"><input type="checkbox" id="spEncToggle" onchange="App.handleEncToggle()">
					<div class="toggle-track"></div><div class="toggle-thumb"></div></label>
			</div>
			<div id="spEncPwRow" style="display:none" class="col">
				<div class="f-grp">
					<div class="f-lbl">Shared password (both peers need the same)</div>
					<input type="password" class="f-in" id="spEncPw" placeholder="Password…">
				</div>
				<button class="btn btn-p btn-full" onclick="App.applyEncKey()">Apply Key</button>
			</div>
			<div id="spEncStatus" style="font-family:'JetBrains Mono',monospace;font-size:.68rem;
				margin-top:8px;color:var(--faint)">Encryption off</div>
		</div>
		<div class="panel-section">
			<div class="panel-section-lbl">Sign In (for Firebase rooms)</div>
			<div id="spAuthArea"></div>
		</div>
		<div class="panel-section">
			<div class="panel-section-lbl">About</div>
			<div style="font-size:.78rem;color:rgba(232,237,248,.4);line-height:1.7">
				P2P Chat v3.0 · ProElectricCoder<br>
	...
	nc.id='newChatModal'; nc.className='modal-overlay';
	nc.innerHTML=`<div class="modal-box">
	<div class="modal-head">
		<span class="modal-title" id="ncTitle">New Chat</span>
		<button class="panel-close" onclick="App.closeNewChat()">✕</button>
	</div>
	...
	<div class="modal-body">
		...
		<!-- Direct tab -->
		<div id="ncDirect">
			<div class="tabs" style="margin-bottom:14px" id="ncDirectSubTabs">
				<button class="tab active" id="ncSubCaller" onclick="App.ncDirectSub('caller')">I'll Call</button>
				<button class="tab"        id="ncSubCallee" onclick="App.ncDirectSub('callee')">I'll Answer</button>
			</div>
			<!-- Caller -->
			<div id="ncCallerFlow" class="col">
				...
				<div class="f-grp hidden" id="ncOfferGroup">
					<div class="f-lbl">Your offer — share this</div>
					<div class="copy-block">
						<textarea class="f-in" id="ncOfferSDP" readonly rows="3" style="padding-right:52px"></textarea>
						<button class="copy-btn" onclick="App.copyField('ncOfferSDP')">copy</button>
					</div>
				</div>
				<div class="f-grp hidden" id="ncAnswerInputGroup">
					<div class="f-lbl">Paste their answer SDP</div>
					<textarea class="f-in" id="ncAnswerInput" placeholder="Paste answer…" rows="3"></textarea>
					<button class="btn btn-p btn-full" onclick="App.directConnect()">Connect</button>
				</div>
			</div>
			<!-- Callee -->
			<div id="ncCalleeFlow" class="col hidden">
				...
				<div class="f-grp">
					<div class="f-lbl">Paste their offer SDP</div>
					<textarea class="f-in" id="ncRemoteOffer" placeholder="Paste offer…" rows="3"></textarea>
				</div>
				<div class="f-grp">
					<div class="f-lbl">Chat name (optional)</div>
					<input class="f-in" id="ncCalleeName" placeholder="Name this chat…">
				</div>
				<button class="btn btn-s btn-full" onclick="App.directGenAnswer()">Create Answer SDP</button>
				<div class="f-grp hidden" id="ncAnswerOutGroup">
					<div class="f-lbl">Your answer — send this back</div>
					<div class="copy-block">
						<textarea class="f-in" id="ncAnswerSDP" readonly rows="3" style="padding-right:52px"></textarea>
						<button class="copy-btn" onclick="App.copyField('ncAnswerSDP')">copy</button>
					</div>
				</div>
			</div>
		</div>
		...
		<!-- Room tab -->
		<div id="ncRoom" class="col hidden">
			<div id="ncRoomAuth"></div>
			<div class="f-grp">
				<div class="f-lbl">Room ID</div>
				<input class="f-in" id="ncRoomId" placeholder="my-room-id">
			</div>
			<div class="row">
				<button class="btn btn-p" style="flex:1" onclick="App.fbCreateRoom()">Create Room</button>
				<button class="btn btn-s" style="flex:1" onclick="App.fbJoinRoom()">Join Room</button>
			</div>
		</div>

		<!-- Group tab -->
		<div id="ncGroup" class="col hidden">
			<div id="ncGroupAuth"></div>
			<div class="f-grp">
				<div class="f-lbl">Group Name</div>
				<input class="f-in" id="ncGroupName" placeholder="My Group">
			</div>
			<div class="f-grp">
				<div class="f-lbl">Room ID (shared with members)</div>
				<div class="row">
					<input class="f-in" id="ncGroupRoomId" placeholder="group-room-id" style="flex:1">
					<button class="btn btn-s" onclick="App.ncGenRoomId()" style="padding:7px 10px;flex-shrink:0">⟳</button>
				</div>
			</div>
			<div class="row">
				<button class="btn btn-p" style="flex:1" onclick="App.fbCreateGroup()">Create Group</button>
				<button class="btn btn-s" style="flex:1" onclick="App.fbJoinGroup()">Join Group</button>
			</div>
		</div>
	...
	co.id='callOverlay'; co.className='call-overlay';
	co.innerHTML=`
	...
	<div class="call-controls">
		...
		<button class="call-ctrl cam hidden" id="callCamBtn" title="Camera off" onclick="App.callToggleCam()">
		...
		<button class="call-ctrl screen hidden" id="callScreenBtn" title="Stop sharing" onclick="App.callStopScreen()">
	...
	ic.id='incomingDialog'; ic.className='incoming-overlay';
	...
	lb.id='lightbox'; lb.onclick=()=>App.closeLightbox();
	...
	tp.id='themePicker';
	tp.innerHTML=`<div class="tp-title">Chat Theme</div>
		<div class="theme-swatches" id="themeSwatches"></div>`;
	document.body.appendChild(tp);
	document.addEventListener('click', e => {
		if (tp.classList.contains('open') && !tp.contains(e.target) && e.target.id!=='btnTheme')
			tp.classList.remove('open');
	});

	// ── CHAT INFO PANEL ──
	const ci = document.createElement('div');
	ci.id='chatInfoOverlay'; ci.className='panel-overlay';
	ci.innerHTML=`<div class="panel-drawer">
	<div class="panel-head">
		<span class="panel-head-title">Chat Info</span>
		<button class="panel-close" onclick="App.closeChatInfo()">✕</button>
	</div>
	<div class="panel-body">
		<div class="panel-section" id="ciDetails"></div>
		<div class="panel-section">
			<div class="panel-section-lbl">Theme</div>
			<div class="theme-swatches" id="ciThemeSwatches"></div>
		</div>
		<div class="panel-section">
			<div class="panel-section-lbl">Members</div>
			<div id="ciMembers" style="font-size:.82rem;color:var(--dim);line-height:1.8"></div>
		</div>
...
function openSettings() {
	...
	// auth
	syncAuthSection('spAuthArea');
	o.classList.add('open');
}

function syncAuthSection(containerId) {
	const a = el(containerId); if (!a) return;
	if (S.user) {
		a.innerHTML = `<div class="auth-card" style="margin-bottom:8px">
			<img class="auth-av" src="${S.user.photoURL||''}" onerror="this.style.display='none'">
			<span class="auth-name">${escH(S.user.displayName||S.displayName)}</span>
		</div>
		<button class="btn btn-d btn-full" onclick="App.signOut()">Sign Out</button>`;
	} else {
		a.innerHTML = `<div class="col">
			<button class="sign-in-btn" onclick="App.signInGoogle()">
				<img src="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 16 16'%3E%3Cpath fill='%23F44336' d='M15.5 8.15c0-.5-.04-1.01-.13-1.5H8v2.84h4.21a3.6 3.6 0 0 1-1.56 2.37v1.97h2.53C14.7 12.35 15.5 10.4 15.5 8.15z'/%3E%3Cpath fill='%234CAF50' d='M8 16c2.14 0 3.94-.71 5.25-1.92l-2.53-1.96c-.71.48-1.62.76-2.72.76-2.09 0-3.86-1.41-4.49-3.3H.9v2.03A7.99 7.99 0 0 0 8 16z'/%3E%3Cpath fill='%23FFC107' d='M3.51 9.58A4.8 4.8 0 0 1 3.26 8c0-.55.09-1.08.25-1.58V4.39H.9A8 8 0 0 0 0 8c0 1.29.31 2.51.9 3.61l2.61-2.03z'/%3E%3Cpath fill='%231565C0' d='M8 3.18c1.17 0 2.23.4 3.06 1.2L13.6 1.8C12.09.4 10.19-.4 8 0a8 8 0 0 0-7.1 4.39l2.62 2.03C4.14 4.6 5.91 3.18 8 3.18z'/%3E%3C/svg%3E" alt="">
				Sign in with Google
			</button>
			<button class="sign-in-btn" onclick="App.signInGitHub()">
...
function openNewChat(tab = 'direct') {
	...
	ncSwitchTab(tab);
	if (tab === 'room'  || tab === 'group') {
		syncAuthSection(tab==='room' ? 'ncRoomAuth' : 'ncGroupAuth');
	}
}
...
function openChatInfo() {
	...
	if (cs) cs.innerHTML = Object.entries(THEMES).map(([id,th])=>
		`<div class="t-sw${sess.theme===id?' active':''}" title="${th.name}"
			style="background:${th.primary}" onclick="App.pickTheme('${id}');App.closeChatInfo()"></div>`
	).join('');
	// Members
	...
	const db = el('ciDisconnectBtn');
	if (db) db.textContent = sess.connected ? 'Disconnect' : 'Reconnect';
...
function initFirebase() {
	firebase.auth().onAuthStateChanged(user => {
		S.user = user;
		if (user) {
			if (!localStorage.getItem('pec_name')) {
				S.displayName = user.displayName || 'Anonymous';
				localStorage.setItem('pec_name', S.displayName);
			}
			S.avatarUrl = user.photoURL || '';
			localStorage.setItem('pec_avatar', S.avatarUrl);
		}
		syncAuthSection('spAuthArea');
		syncAuthSection('ncRoomAuth');
		syncAuthSection('ncGroupAuth');
	});
}
...
function bufB64(ab) {
	const u8=new Uint8Array(ab); let s=''; const B=8192;
...
function toast(msg, ms=3000) {
	...
	clearTimeout(_toastTmr); _toastTmr=setTimeout(()=>e.classList.remove('show'),ms);
}

// ═══════════════════════════════════════════════════════════════════════════
// 17. window.App PUBLIC API
// ═══════════════════════════════════════════════════════════════════════════
window.App = {
	// Sidebar (mobile)
	...
	async deleteChat(id) { if (confirm('Delete this chat and all its messages?')) await deleteSess(id); },
	pickTheme(id) {
		const sess=getActiveSess(); if(!sess) return;
		setThemeForSess(sess.id, id);
	...
	ncGenRoomId() {
		const inp=el('ncGroupRoomId');
		if(inp) inp.value='room-'+Math.random().toString(36).slice(2,8);
	},
	...
	async directGenOffer() {
		...
		NC.pendingEngine=eng; NC.pendingSessId=sessId;
		bindEngine(sess, eng);
		...
		try {
			...
			renderChatList();
			toast('Offer generated — share it with your peer');
		} catch(e) { toast('Error: '+e.message); S.sessions.delete(sessId); }
	},
	async directConnect() {
		const ans=el('ncAnswerInput')?.value.trim();
		if (!ans) { toast('Paste the answer SDP first'); return; }
	...
	async directGenAnswer() {
		const offer=el('ncRemoteOffer')?.value.trim();
		if (!offer) { toast('Paste the remote offer first'); return; }
		const name=(el('ncCalleeName')?.value.trim()||'Direct Chat');
		const eng=new DirectEngine();
		const sess=makeSess({ name, type:'direct' });
		S.sessions.set(sess.id, sess);
		bindEngine(sess, eng);
		try {
			const answer=await eng.createAnswerFor(offer);
			el('ncAnswerSDP').value=answer;
			el('ncAnswerOutGroup')?.classList.remove('hidden');
			await DB.saveSession(sess);
			renderChatList();
			toast('Answer generated — send it back');
	...
	async fbCreateRoom() {
		...
		S.sessions.set(sess.id, sess);
		bindEngine(sess, eng);
		setStatus('connecting','Waiting for peers…');
		try {
			await eng.createRoom(rid);
			await DB.saveSession(sess);
			await selectSess(sess.id);
			this.closeNewChat();
			addSysMsg(sess, `Room "${rid}" created — share this ID`);
			toast('Room created');
		} catch(e) { toast('Error: '+e.message); S.sessions.delete(sess.id); }
	},
	async fbJoinRoom() {
		if (!S.user) { toast('Sign in first'); return; }
		const rid=el('ncRoomId')?.value.trim();
	...
	async fbCreateGroup() {
		...
		sess.isHost=true;
		const eng=new ChatEngine({ relay:true });
		eng.init(firebase.firestore());
		S.sessions.set(sess.id, sess);
		bindEngine(sess, eng);
		try {
			...
			await selectSess(sess.id);
			this.closeNewChat();
			addSysMsg(sess, `Group "${gname}" created · Room: ${rid}`);
			toast('Group created');
		} catch(e) { toast('Error: '+e.message); S.sessions.delete(sess.id); }
	},
	async fbJoinGroup() {
		...
		const sess  =makeSess({ name:gname, type:'firebase', isGroup:true, roomId:rid, groupName:gname });
		const eng   =new ChatEngine({ relay:false });
		eng.init(firebase.firestore());
		S.sessions.set(sess.id, sess);
		bindEngine(sess, eng);
		try {
	...
	async sendMsg() {
		...
		if (S.encEnabled && Crypt.key) {
			try { payload=await Crypt.encText(text); enc=true; }
			catch(e) { toast('Encrypt error: '+e.message); return; }
		}
	...
	handleFileSelect(fs)  { [...fs].forEach(f=>sendFile(f)); },
	handleDrop(e)         { e.preventDefault(); [...(e.dataTransfer.files||[])].forEach(f=>sendFile(f)); },
	...
	callEnd()            { const s=S.sessions.get(S.callSessId); endCallInternal(s,true); addSysMsg(s,'Call ended'); },
	callToggleMute()     { toggleCallMute(); },
	callToggleCam()      { toggleCallCam(); },
	callStopScreen()     { stopScreenShare(); },

	// Settings
	openSettings()  { openSettings(); },
	closeSettings() { el('settingsOverlay')?.classList.remove('open'); },
	saveName() {
		const n=el('spName')?.value.trim()||'Anonymous';
		S.displayName=n; localStorage.setItem('pec_name',n);
		const sess=getActiveSess();
		if (sess?.connected) safeSend(sess, { type:'display-name', displayName:n });
		toast('Name saved');
	},
	handleEncToggle() {
	...
	async applyEncKey() {
		...
		try {
			await Crypt.derive(pw); S.encEnabled=true;
			el('spEncStatus').textContent='🔒 Key active'; el('spEncStatus').style.color='var(--ta)';
...
(async function init() {
	...
	} catch { /* gradient falls back to linear */ }
